...
This training script is based on the train_v01
initialize with certain layers with pretrained emotion model
"""
...
# Model options
parser.add_argument('--ModelName', type=str, default='model_cnn_seq_CMU_pretrained_N2',
...
args.expName = os.path.join('ckpts_couple_seq_emotion_pretrained_binary_trainNo2', args.expName, args.testCoupleID)
...
NN_top_couple_class_module = importlib.import_module('Model_class_sequence_couple_pretrained_CMU')
...
def main():
    ...
    logging.info("Number of params before freeze is {}".format(num_params))
    
    
    # init with pretrained models
    #-------------------------------
    ...
    pretrained_dict_remained = {k: v for k, v in pretrained_dict.items() if k in curr_whole_model_dict}
    curr_whole_model_dict.update(pretrained_dict_remained)
    ...
    init_lr = args.lr
    # only optimize models with requires_grad= True
    optimizer = torch.optim.Adam(filter(lambda p: p.requires_grad, model.parameters()), lr=init_lr)
    
    num_params = count_parameters(model)
    logging.info("Number of params after freeze is {}".format(num_params))
