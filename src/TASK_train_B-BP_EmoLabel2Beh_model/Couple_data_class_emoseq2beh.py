...
# -*- coding: utf-8 -*-
"""
Created on Sun Jan  6 20:06:01 2019
    ...
    def load_couple_info(self, rating_info_npy, binary_label_info_npy,\
        ...
        for file in file_list:
            ...
            if couple_id in train_couple_id_lst:
                session_train_list.append('hus.'+file)
                session_train_list.append('wife.'+file)
            if couple_id in dev_couple_id_lst:
                # dev
                session_dev_list.append('hus.'+file)
                session_dev_list.append('wife.'+file)
            if couple_id in test_couple_id_lst:
                # test
                session_test_list.append('hus.'+file)
                session_test_list.append('wife.'+file)
        
        session_train_list_pair = [(x, binary_score_dict[x]) for x in session_train_list if x in all_couple_rating_dict]
        session_dev_list_pair =[(x, binary_score_dict[x]) for x in session_dev_list if x in all_couple_rating_dict]
        session_test_list_pair = [(x, binary_score_dict[x]) for x in session_test_list if x in all_couple_rating_dict]
