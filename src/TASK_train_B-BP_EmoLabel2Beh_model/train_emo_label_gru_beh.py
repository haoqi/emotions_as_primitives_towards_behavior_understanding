...
# -*- coding: utf-8 -*-
"""
Created on Sun Jan  6 20:02:48 2019

@author: haoqi

training script to map emotion labels to beh scores

In this setting, use batch_sz = 1, so we do not need to do the padding
...
from Couple_data_class_emoseq2beh import Couple_info_class_binary_w_mask_split_dev

from Dataloader_couple_emo_label_seq import Dataloader_couple_emotion_label_seq
...
# Model options
parser.add_argument('--ModelName', type=str, default='Emotion_Seq2Beh_Model',
...
args.expName = os.path.join('ckpts_couple_emo_seq_binary_trainNo1', args.expName, args.testCoupleID)
...
NN_top_couple_class_module = importlib.import_module('Model_EmoSeq2Beh')
...
def train_iter(model, optimizer, criterion, epoch, data_feat_torch, target_label_torch, mask, total_non_masked_samples, threshold):
    #input data_feat_torch [batch=1, length, 6]
    
    model.train()
    data_feat_torch = data_feat_torch.permute(1,0,2).to(device)
...
def main():
    ...
    dataset_train_torch = Dataloader_couple_emotion_label_seq(couple_info_class.gen_train_list())
    dataset_dev_torch = Dataloader_couple_emotion_label_seq(couple_info_class.gen_dev_list())
    dataset_test_torch = Dataloader_couple_emotion_label_seq(couple_info_class.gen_test_list())
    ...
    model = selected_model_name().to(device)
    ...
    for epoch in range(start_epoch, start_epoch + args.epochs):
        ...
        for iter_idx, (feat_in_torch, label_in_torch, binary_label_mask, len_mask, name) in enumerate(dataloader_train):
            # feat_in_torch: shape[batch=1, length, 6]
